...
public class PasswordResetDAO {
    ...
    public void guardarToken(int idUsuario, String token) {
        ...
        String query = "INSERT INTO password_reset(id_usuario, fecha_hora_creacion, token, utilizado) " +
                "VALUES (?, NOW(), ?, FALSE)";
        ...
        try (
            ...
            logger.error("Error al guardar token para usuario ID {} - Estado: {} - Código: {}",
        ...
        try (
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    return mappingPasswordReset(rs);
                } else {
            ...
            logger.error("Error al obtener token {} - Estado: {} - Código: {}",
    ...
    public void marcarComoUtilizado(String token) {
        ...
        try (
            ...
            logger.info("Token marcado como utilizado: {}", token);

        } catch (SQLException e) {
            logger.error("Error al marcar token {} - Estado: {} - Código: {}",
                    token, e.getSQLState(), e.getErrorCode(), e);
    ...
    public boolean existeTokenPendiente(int idUsuario) {
        logger.debug("Verificando tokens pendientes para usuario ID: {}", idUsuario);
