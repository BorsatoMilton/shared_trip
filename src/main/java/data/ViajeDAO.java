...
public class ViajeDAO {
    ...
    public void updateCantidad(int idViaje, int cantPasajeros) {
        ...
        try {
            conn = ConnectionDB.getInstancia().getConn();
            stmt = conn.prepareStatement("UPDATE viajes SET lugares_disponibles = ? WHERE id_viaje = ?");
    ...
    public boolean cancelarViaje(int id_viaje) {
        ...
        try {
            conn = ConnectionDB.getInstancia().getConn();
            stmt = conn.prepareStatement("UPDATE viajes SET cancelado = true WHERE id_viaje = ?");
            stmt.setInt(1, id_viaje);
    ...
    public void update(Viaje v, int id_viaje) {
        ...
        try {
            ...
            stmt = conn.prepareStatement(
                "UPDATE viajes SET fecha=?, lugares_disponibles=?, origen=?, destino=?, " +
    ...
    public void add(Viaje v) {
        ...
        try {
            conn = ConnectionDB.getInstancia().getConn();
            stmt = conn.prepareStatement(
                "INSERT INTO viajes(fecha, lugares_disponibles, origen, destino, precio_unitario, " +
    ...
    public void delete(Viaje v) {
        ...
        try {
            conn = ConnectionDB.getInstancia().getConn();
            stmt = conn.prepareStatement("DELETE FROM viajes WHERE id_viaje = ?");
    ...
    /**
     * Mapeo desde JOIN - NO llama a otros DAOs
     */
    private Viaje mapViajeFromJoin(ResultSet rs, boolean puntuar) throws SQLException {
        ...
        // Vehículo COMPLETO desde el JOIN
